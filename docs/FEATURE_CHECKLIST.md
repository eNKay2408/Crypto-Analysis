...
| ID     | Feature             | Screen (UI Tóm tắt)               | API / Backend Logic (Tóm tắt)                                                                  | Owner    | Status                                     |
| :----- | :------------------ | :-------------------------------- | :--------------------------------------------------------------------------------------------- | :------- | :----------------------------------------- |
| **F6** | **Real-time WS**    | Chart tự nhảy giá (nến giật)      | WS Server: Subscribe Binance -> Broadcast to Client. Logic `isFinal` xử lý tại Client hoặc BE. | Mem 1, 2 | ✅ DONE (WS relay service + FE integration) |
| **F7** | **Sentiment AI**    | (Background Worker)               | Trigger khi có tin mới -> Gọi Model Sentiment -> Update field `sentiment_score` trong DB.      | Mem 3    | ✅ DONE (FinBERT + NER + ChangeStream)      |
| **F8** | **News Admin**      | Trang quản lý Source (Add/Remove) | UI gọi API `F5`. Hiển thị list source đang crawl.                                              | Mem 2, 1 | ⬜ TODO                                     |
| **F9** | **Visualized News** | Màu sắc tin tức trên Chart        | FE: Vẽ marker lên chart tại thời điểm có tin tức (Xanh/Đỏ theo sentiment).                     | Mem 2    | ⬜ TODO                                     |
